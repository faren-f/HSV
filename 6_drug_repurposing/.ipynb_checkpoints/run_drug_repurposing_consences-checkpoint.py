...
# Each module must contain a protein file (TSV) with a gene column.
...
# python /home/bbc8731/HSV/6_drug_repurposing/run_drug_repurposing_consences.py /home/bbc8731/HSV/3_module_expansion/data/categories_methods /home/bbc8731/HSV/6_drug_repurposing/data/drugs4validation.txt /home/bbc8731/HSV/6_drug_repurposing/data/drugs4validation.txt /home/bbc8731/HSV/6_drug_repurposing/data/random_drugs_DrugsVirus.txt
...
class NeDRexDrugRepurposing:
    def __init__(self, base_dir: Path, true_drugs_files: Dict[str, Path], base_url: str = "https://api.nedrex.net"):
        ...
        # self.methods = ['amygdala', 'brain', 'dorsal_root_ganglia', 'hypothalamus', 'salivary_gland', 'uniprot_ppi']
        self.methods = ['uniprot_ppi']
    ...
    def _read_true_drugs(self, file_path: Path) -> List[str]:
        """Helper tissue to read and process drug IDs from file"""
        drugs = []
    ...
    def build_drug_protein_network(self):
        ...
        try:
            ...
            payload = {
                ...
                "drug_groups": ["approved", "experimental"],
                "taxid": [9606]
            }
    ...
    def process_module(self, module_dir: Path):
        ...
        try:
            ...
            nodes_dir = module_dir / 'consensus'

            # Process all methods silently
            for tissue in self.methods:
                self._process_method(tissue, nodes_dir, module_dir, repurposing_dir)

        except Exception as e:
            self.logger.error(f"Failed to process {module_dir.name}: {str(e)}")

    def _process_method(self, tissue: str, nodes_dir: Path, module_dir: Path, repurposing_dir: Path):
        """Helper tissue to process a single tissue"""

        node_file = nodes_dir / f"{tissue}.tsv"


        if not node_file.exists():
            self.logger.warning(f"Node file not found for {tissue}: {node_file}")
            return

        self.logger.info(f"Processing tissue: {tissue}")
        proteins = self.read_node_file(node_file)
        
        proteins = ['uniprot.' + p if not p.startswith('uniprot.') else p for p in proteins]

        for algorithm in self.algorithms:
            self._run_algorithm(algorithm, tissue, proteins, repurposing_dir)


    def _run_algorithm(self, algorithm: str, tissue: str, proteins: List[str], repurposing_dir: Path):
        """Helper tissue to run a single algorithm"""
        self.logger.info(f"Running {algorithm} for {tissue}")
        
        method_dir = repurposing_dir / algorithm / tissue
        method_dir.mkdir(parents=True, exist_ok=True)


        try:
            uid = self.submit_algorithm(algorithm, proteins)
            self._monitor_job(uid, algorithm, tissue, method_dir)
        except Exception as e:
            self.logger.error(f"Error running {algorithm} for {tissue}: {str(e)}")
            self._save_error_info(method_dir, str(e), tissue, algorithm, len(proteins))

    def _run_validations(self, predicted_drugs: List[str], method_dir: Path, tissue: str):
    ...
    def _submit_validation(self, predicted_drugs: List[str], validation_dir: Path,
        ...
        try:
            ...
            predicted_pairs = [[f"drugbank.{d[0].replace('drugbank.', '')}", float(d[1])] for d in predicted_drugs]
            ...
            #######
            pred_set = set(d[0].replace('drugbank.', '') for d in predicted_drugs)

            # pred_set = set(d.replace('drugbank.', '') for d in predicted_drugs)
            true_set = set(d.replace('drugbank.', '') for d in true_drugs_clean)
            
            tp = len(pred_set & true_set)
            recall = tp / len(true_set) if len(true_set) > 0 else 0.0
            
            self.logger.info(
                f"[{method_name}][{drug_set_name}] Recall = {recall:.4f} ({tp}/{len(true_set)})")
            ...
            while True:
                ...
                if status['status'] == 'completed':
                    ...
                    results["num_true"] = len(true_set)
                    results["num_predicted"] = len(pred_set)
    ...
    def _monitor_job(self, uid: str, algorithm: str, tissue: str, method_dir: Path):
        """Monitor job progress with status updates"""
        start_time = time.time()
        last_update = 0
        pbar = tqdm(total=100, desc=f"{algorithm} for {tissue}", unit="%")
        ...
        while True:
            ...
            if current_time - last_update >= 60:
                self.logger.info(f"{algorithm} for {tissue}: {status} ({elapsed_minutes:.1f} min)")
                last_update = current_time

            if status == 'completed':
                pbar.update(100 - pbar.n)
                pbar.close()
                self._handle_completed_job(status_response, method_dir, tissue)
                break
            elif status == 'failed':
                pbar.close()
    ...
    def _handle_completed_job(self, status_response: Dict, method_dir: Path, tissue: str):
        """Helper tissue to handle completed job"""

        if 'results' in status_response:
            results = status_response['results']
            results_file = method_dir / "results.json"
            print(results['drugs'][:5])
            ...
            self._run_validations(predicted_drugs, method_dir, tissue)


    def _extract_predicted_drugs(self, results: Dict) -> List[List]:
        if isinstance(results, dict) and 'drugs' in results:
            return [[drug['drug_name'],float(drug.get('score', 1.0))]
    ...
    #     """Helper tissue to extract predicted drugs from results"""
    #     if isinstance(results, dict) and 'drugs' in results:
    ...
    def _save_error_info(self, method_dir: Path, error: str, tissue: str,
                         algorithm: str, num_proteins: int):
        """Helper tissue to save error information"""
        error_info = {
            'error': error,
            'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
            'method': tissue,
...
def main():
    if len(sys.argv) != 5:  # Updated to accept one more argument
        print("Usage: script.py <base_directory> <approved_drugs_file> <approved_experimental_drugs_file> <random_drugs_file>")
        sys.exit(1)

    base_dir = Path(sys.argv[1])
    true_drugs_files = {
        "approved_drugs": Path(sys.argv[2]),
        "approved_experimental_drugs": Path(sys.argv[3]),
        "random_drugs": Path(sys.argv[4])  # Added the random drugs file
    }
